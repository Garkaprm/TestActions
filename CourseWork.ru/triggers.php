...
<html>
   ...
   ?>
    ...
    <body style="margin: 0; overflow-y: scroll; background: #c5c5c5;" >
            ...
            <content>
                <br><br>
                <p style="text-align: center"> Введите Ваш запрос на добавление триггера<br><br>
                <form action="sql.php" method= "POST">
                    <input name="Text" type="text" style="height: 30px; width: 100%;" size="100" placeholder="CREATE TRIGGER Название_триггера [AFTER или BEFORE] [INSERT, DELETE, UPDATE] ON Название_таблицы FOR EACH ROW [INSERT INTO log (action, date, time) VALUES ('insert to adresses', CURRENT_DATE, CURRENT_TIME)]">
                    <input type="submit" class="submit" style="padding: 10px 20px; float: right" value= "Выполнить"><br><br>
                </form>
                <p style="text-align: center"><b>Существующие в базе данных <?print $_SESSION['db'] ?> триггеры:</b></p><hr><br>
                <?php
                {
                    $result = mysqli_query($connection, "SHOW TRIGGERS");
                    $count = 1;
                    while ($row = mysqli_fetch_row($result))
                    {
                        foreach($row as $val)

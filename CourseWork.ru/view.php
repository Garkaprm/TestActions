...
<html>
    <?php
        ...
        function showview ( $val)
        {
            ...
            mysqli_query($connection1, "USE '$val'");
            $result = mysqli_query($connection1, "SELECT TABLE_NAME FROM information_schema.`TABLES` WHERE TABLE_TYPE LIKE 'VIEW' AND TABLE_SCHEMA LIKE '$val'");
            while ($row = mysqli_fetch_row($result))
            {
                foreach($row as $val1)
                {
                    ?>
                    <form method="post">
                        <input name="tab" type="submit" class="db" value="<? print $val1;?>" formaction="tables.php">
                    </form>
                    <?
                }
            }
   ...
   ?>
    ...
    <body style="margin: 0; overflow-y: scroll; background: #c5c5c5;" >
            ...
            <content>
                <br><br>
                <p style="text-align: center"> Введите Ваш запрос на добавление представления<br>
                <br><br>
                <form action="sql.php" method= "POST">
                    <input name="Text" type="text" style="height: 30px; width: 100%;" size="100" placeholder="CREATE OR REPLACE VIEW Имя_таблицы (Поле_1, Поле_2, ...) AS [запрос SELECT]">
                    <input type="submit" class="submit" style="padding: 10px 20px; float: right" value= "Выполнить"><br><br>
                </form>
                <br><p style="text-align: center"><b>В базе данных содержатся следующие представления:</b></p><hr><br>
                <?php
                if (isset($_POST))
                {
                    showview($_SESSION['db']);
                }?>
            </content>
    </body>
</html>